...
# 1. Analysing the Joining Questionnaire{#S2}
...
**Exercise 1.1:**  Why bother to write R code at all, surely it will be easier to just use a spreadsheet?
...
Remove column X as this is redundant
...
**Extension exercise 1.7:** Is there any relationship between `Background` [UG course] and choice of MSc?
...
**Exercise 1.8:** Do AI and DSA students have different perceived levels of statistical and programming understanding?
...
**Exercise 1.10:** What are the dominant themes for (i) `Excite` and (ii) `Concern`?  Does the word cloud also find unimportant words?  What about similar words but with different endings e.g., singular and plural?
...
height <- round(rnorm(n = 20, mean = 1.65, sd = 0.15), 3)   # The results are rounded for readability
```
...
**Exercise 2.4:** Add `newRow` to the end of  dataframe `newDF`.  
